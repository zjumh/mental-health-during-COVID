...
# filter the tweets from healthcare workers
def hcws_select(data):
    prefix = "( |,|;|\.|\!|^)"
    suffix = "( |,|;|\.|\!|$)"
    doctor_pattern = "(doctor|physician|surgeon|dentist|ophthalmologist|pediatrician|gynecologist|nurse|Physician|Surgeon|Dentist|Ophthalmologist|Pediatrician|Gynecologist|Nurse)"
    md_pattern = "(MD|BN|B.S.Med|BSMT|B.S.Med.Tech|BSN|BSNurs|MSMT|MSN|DDS|MBBS|MHS|MPH|RPH|RN|FACP|FACS|FICS|FRCOG|FRCP)"
    data['doctor'] = data['user'].str.contains(prefix + doctor_pattern + suffix, na=False, case=True)
    data['medicine'] = data['user'].str.contains(
        ...
        na=False)
    data['MD'] = data['user'].str.contains(prefix + md_pattern + suffix, na=False, case=True)
